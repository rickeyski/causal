...
class OAuthClient(oauth.Client):
    """Code lifed from the excellent: 
    https://github.com/simplegeo/python-oauth2/blob/master/example/client.py"""

    def __init__(self, settings):
        #server, port=httplib.HTTP_PORT, request_token_url='', access_token_url='', authorization_url=''):
        self.server = 'localhost' # us
        self.port = '8080' # FIXME !
        self.request_token_url = settings['auth']['request_token_url']
        self.access_token_url = settings['auth']['access_token_url']
        self.authorization_url = settings['auth']['authorization_url']
